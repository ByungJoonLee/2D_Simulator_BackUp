...
void POISSON_SOLVER::Initialize(const T& tolerance_input, const int& max_itr_input, const int ghost_width_input = 0, MULTITHREADING* multithreading_input = 0)
{
...
void POISSON_SOLVER::InitializePressure(FIELD_STRUCTURE_1D<T>& pressure)
{
	int i;
...
void POISSON_SOLVER::InitializePressure(FIELD_STRUCTURE_2D<T>& pressure)
{
	int i, j;
...
void POISSON_SOLVER::InitializePressure(FIELD_STRUCTURE_1D<T>& pressure, const int& thread_id)
{
...
void POISSON_SOLVER::InitializePressure(FIELD_STRUCTURE_2D<T>& pressure, const int& thread_id)
{
...
// Solver
void POISSON_SOLVER::Solve(FIELD_STRUCTURE_1D<T>& pressure, FIELD_STRUCTURE_1D<T>& density, FIELD_STRUCTURE_1D<int>& bc, const FIELD_STRUCTURE_1D<T>& div, const FIELD_STRUCTURE_1D<T>& variable, const LEVELSET_1D& levelset, const FIELD_STRUCTURE_1D<T>& jc_on_solution, const FIELD_STRUCTURE_1D<T>& jc_on_derivative, const int& thread_id)
{
...
void POISSON_SOLVER::Solve(FIELD_STRUCTURE_2D<T>& solution, FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& b_vector, const int& thread_id)
{
	...
	BuildLinearSystem(A, x, b, solution, bc, b_vector, thread_id);

	linear_solver->Solve(A, x, b, bc);
	VectorToGrid(x, solution, bc, thread_id);
}

void POISSON_SOLVER::Solve(FIELD_STRUCTURE_2D<T>& pressure, FIELD_STRUCTURE_2D<T>& density, FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& div, const LEVELSET_2D& levelset, const FIELD_STRUCTURE_2D<T>& jc_on_solution, const FIELD_STRUCTURE_2D<T>& jc_on_derivative)
{
	assert(linear_solver != 0);
...
void POISSON_SOLVER::Solve(FIELD_STRUCTURE_2D<T>& pressure, FIELD_STRUCTURE_2D<T>& density, FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& div, const FIELD_STRUCTURE_2D<T>& variable, const LEVELSET_2D& levelset, const FIELD_STRUCTURE_2D<T>& jc_on_solution, const FIELD_STRUCTURE_2D<T>& jc_on_derivative, const int& thread_id)
{
	assert(linear_solver != 0);
...
void POISSON_SOLVER::SolveForAxisymmetric(FIELD_STRUCTURE_2D<T>& pressure, const FIELD_STRUCTURE_2D<T>& coef_1, const FIELD_STRUCTURE_2D<T>& coef_2, const FIELD_STRUCTURE_2D<T>& coef_3, const FIELD_STRUCTURE_2D<T>& coef_4, FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& div)
{
	assert(linear_solver != 0);
...
// Member Functions
void POISSON_SOLVER::SetTolerance(const T& tolerance_input)
{
...
void POISSON_SOLVER::BuildLinearSystemNodeForAxiSymmetric(CSR_MATRIX<T>& A_matrix, VECTOR_ND<T>& x_vector, VECTOR_ND<T>& b_vector, const FIELD_STRUCTURE_2D<T>& coef_1, const FIELD_STRUCTURE_2D<T>& coef_2, const FIELD_STRUCTURE_2D<T>& coef_3, const FIELD_STRUCTURE_2D<T>& coef_4, const FIELD_STRUCTURE_2D<T>& pressure, const FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& div)
{
...
void POISSON_SOLVER::BuildLinearSystem(CSR_MATRIX<T>& A_matrix, VECTOR_ND<T>& x_vector, VECTOR_ND<T>& b_vector, const FIELD_STRUCTURE_2D<T>& solution, const FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& RHS, const int& thread_id)
{
	...
	BEGIN_GRID_ITERATION_2D(solution.partial_grids[thread_id])
	{
		...
		{
			coef_ijk += 1;
			A_matrix.AssignValue(bc(i, j), bc(i - 1, j), -inv_dxdx, thread_id);
		}
		if (bc(i + 1, j) > -1)
		{
			coef_ijk += 1;
		...
		{
			coef_ijk += 1;
			A_matrix.AssignValue(bc(i, j), bc(i, j - 1), -inv_dydy, thread_id);
		}
		if (bc(i, j + 1) > -1)
		{
			coef_ijk += 1;
		...
		if (bc(i - 1, j) == BC_IMPLICIT)
		{
			T x_coor = bc.grid.x_min + i*dx;
			
			b_vector[bc(i, j)] -= one_over_dx*x_coor;
		}
		if (bc(i + 1, j) == BC_IMPLICIT)
		{
			T x_coor = bc.grid.x_min + i*dx;
		...
		if (bc(i, j - 1) == BC_IMPLICIT)
		{
			T y_coor = bc.grid.x_min + j*dx;
			
			b_vector[bc(i, j)] -= one_over_dy*y_coor;
		}
		if (bc(i, j + 1) == BC_IMPLICIT)
		{
			T y_coor = bc.grid.x_min + j*dx;
...
void POISSON_SOLVER::BuildLinearSystemNodeForSemiImplicitViscosity(CSR_MATRIX<T>& A_matrix, VECTOR_ND<T>& x_vector, VECTOR_ND<T>& b_vector, const FIELD_STRUCTURE_2D<T>& coef_1, const FIELD_STRUCTURE_2D<T>& coef_2, const FIELD_STRUCTURE_2D<T>& coef_3, const FIELD_STRUCTURE_2D<T>& coef_4, const FIELD_STRUCTURE_2D<T>& coef_5, const FIELD_STRUCTURE_2D<T>& velocity, const FIELD_STRUCTURE_2D<int>& bc, const FIELD_STRUCTURE_2D<T>& explicit_term)
{
	const int num_all_full_cells = AssignSequentialindicesToFullCells(bc);
	const int nnz = CountNonZeroElements(bc);
...
void POISSON_SOLVER::BuildLinearSystemNodeJumpConditionVaribleCoefficient(CSR_MATRIX<T>& A_matrix, VECTOR_ND<T>& x_vector, VECTOR_ND<T>& b_vector, const FIELD_STRUCTURE_1D<T>& pressure, const FIELD_STRUCTURE_1D<T>& variable, const FIELD_STRUCTURE_1D<int>& bc, const FIELD_STRUCTURE_1D<T>& div, const LEVELSET_1D& levelset, const FIELD_STRUCTURE_1D<T>& jc_on_solution, const FIELD_STRUCTURE_1D<T>& jc_on_derivative, const int& thread_id)
{
	const int num_all_full_cells = AssignSequentialindicesToFullCells(bc, thread_id);
	const int nnz = CountNonZeroElements(bc, thread_id);
...
int POISSON_SOLVER::AssignSequentialindicesToFullCells(const FIELD_STRUCTURE_2D<int>& bc)
{
...
int POISSON_SOLVER::AssignSequentialindicesToFullCells(const FIELD_STRUCTURE_1D<int>& bc, const int& thread_id)
{
	// Count number of full cells
	int full_ix(0);
...
int POISSON_SOLVER::AssignSequentialindicesToFullCells(const FIELD_STRUCTURE_2D<int>& bc, const int& thread_id)
{
	// Count number of full cells
	int full_ix(0);
...
int POISSON_SOLVER::CountNonZeroElements(const FIELD_STRUCTURE_2D<int>& bc)
{
	int nnz(0);
...
int POISSON_SOLVER::CountNonZeroElements(const FIELD_STRUCTURE_1D<int>& bc, const int& thread_id)
{
	int nnz(0);
...
int POISSON_SOLVER::CountNonZeroElements(const FIELD_STRUCTURE_2D<int>& bc, const int& thread_id)
{
	int nnz(0);
...
void POISSON_SOLVER::VectorToGrid(const VECTOR_ND<T>& x, FIELD_STRUCTURE_2D<T>& pressure, const FIELD_STRUCTURE_2D<int>& bc)
{
	int i, j;
	LOOPS_2D(i, j, pressure.i_start, pressure.j_start, pressure.i_end, pressure.j_end)
	{
...
void POISSON_SOLVER::VectorToGrid(const VECTOR_ND<T>& x, FIELD_STRUCTURE_1D<T>& pressure, const FIELD_STRUCTURE_1D<int>& bc, const int& thread_id)
{
	BEGIN_GRID_ITERATION_1D(pressure.partial_grids[thread_id])
	{
...
void POISSON_SOLVER::VectorToGrid(const VECTOR_ND<T>& x, FIELD_STRUCTURE_2D<T>& pressure, const FIELD_STRUCTURE_2D<int>& bc, const int& thread_id)
{
	BEGIN_GRID_ITERATION_2D(pressure.partial_grids[thread_id])
	{
		pressure.array_for_this(i, j) = x[bc(i, j)];
